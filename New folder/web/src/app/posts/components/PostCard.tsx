        ...
        <div className="flex justify-center w-full">
            <div className="w-full max-w-2xl bg-white rounded-xl p-4 shadow-sm mb-6">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {allMedia.slice(0, maxShow).map((url, idx) => {
                                        ...
                                        return (
                                            <div
                                                key={idx}
                                                className={`${isFirst ? "col-span-2" : ""} relative h-[180px] overflow-hidden rounded-lg`}
                                            >
                                                {isVid ? (
                                                    <video
                                                        src={url}
                                                ...
                                                {/* Hiển thị overlay số +n trên ảnh cuối */}
                                                {isLast && !isVid && (
                                    ...
                                    {/* Ẩn thêm ảnh để có thể xem preview toàn bộ */}
                ...
                <div className="flex justify-between text-xs text-gray-500 mt-3" >
                    ...
                    >
                        <button
                            onClick={() => handleFeelClick(userFeel === 'like' ? 'haha' : 'love')}
          ...
          3.78-3.4 6.86-8.55 11.54L12 21.35z" />
                            ...
                            {userFeel === 'love' && (
                                <span className="text-red-600 text-lg select-none" aria-label="love">😍</span>
                            )}
                            {userFeel === 'haha' && (
                                <span className="text-yellow-500 text-lg select-none" aria-label="haha">😂</span>
                            )}
                            {!userFeel && (
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
          ...
          3.78-3.4 6.86-8.55 11.54L12 21.35z" />
                            ...
                            <span className="select-none">
                                {userFeel === 'like' && `${totalFeelCount} All${totalFeelCount !== 1 ? 's' : ''}`}
                                {userFeel === 'love' && `${totalFeelCount} All${totalFeelCount !== 1 ? 's' : ''}`}
                                {userFeel === 'haha' && `${totalFeelCount} All${totalFeelCount !== 1 ? 's' : ''}`}
                        ...
                        {showFeelOptions && (
                            <div
                                className="absolute  left-0 flex gap-3 bg-white rounded-xl shadow-lg p-2 z-20 min-w-[130px]"
                            >
                                {FEELS.map(({ type, icon }) => (
                                    <button
                                        key={type}
                                        onClick={() => handleFeelClick(type as 'like' | 'love' | 'haha')}
                                        className={clsx(
