...
import { FiEdit, FiTrash2 } from "react-icons/fi"; // Thêm icon edit/delete
import clsx from "clsx";
import { PhotoProvider, PhotoView } from "react-photo-view";
import "react-photo-view/dist/react-photo-view.css";
import { formatDate } from "@/utils";
import { BsGlobeAmericas, BsPeopleFill } from "react-icons/bs";
import { PostCardProps, PostFromServer, PostResponse } from "../../type";
import { deletePost, getPostId, updatePostFeel } from "../../user.service";
...
const PostCard: React.FC<PostCardProps> = ({
    ...
    const handleDelete = async (postId: string) => {
        if (confirm("You sure want to delete this post?")) {
        ...
        <div className="flex justify-center w-full">
            <div className="w-full max-w-2xl bg-white rounded-xl p-4 shadow-sm mb-6">
                ...
                <div className="text-sm text-gray-700 whitespace-pre-line mt-2">
                    ...
                    {Array.isArray(friends_tagged) && friends_tagged.length > 0 && (
                        <div className="mt-2 flex flex-wrap gap-2">
                            {friends_tagged.map((friend, idx) => (
                                <span
                                    key={idx}
          ...
          3.78-3.4 6.86-8.55 11.54L12 21.35z"
                            ...
                            <span className="select-none">
                                {userFeel === "like" &&
                                    `${totalFeelCount} Like${totalFeelCount !== 1 ? "s" : ""}`}
                                {userFeel === "love" &&
                                    `${totalFeelCount} Love${totalFeelCount !== 1 ? "s" : ""}`}
                                {userFeel === "haha" &&
                                    `${totalFeelCount} Haha${totalFeelCount !== 1 ? "s" : ""}`}
